...
func _draw() -> void:
	...
	draw_set_transform(center - focus_position * ZOOM, 0.0, Vector2(ZOOM, ZOOM))
	GameConfig.draw_terrain(self)
	draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE) # back to window-local space for everything below
	...
	if unit_kind == Unit.Kind.MORTAR:
		draw_circle(center, TOKEN_RADIUS * 0.45, Color(0.0, 0.0, 0.0, TOKEN_ALPHA))
	elif unit_kind == Unit.Kind.SPOTTER or unit_kind == Unit.Kind.DRONE_TEAM:
		draw_circle(center, TOKEN_RADIUS * 0.4, Color(0.1, 0.1, 0.1, TOKEN_ALPHA))
		draw_circle(center, TOKEN_RADIUS * 0.18, Color(1.0, 1.0, 1.0, TOKEN_ALPHA))
	GameConfig.draw_cover_ring(self, TOKEN_RADIUS, GameConfig.get_terrain_type_at(focus_position), center)
	...
	draw_line(center - Vector2(6, 0), center + Vector2(6, 0), crosshair_color, 1.0)
	draw_line(center - Vector2(0, 6), center + Vector2(0, 6), crosshair_color, 1.0)

	draw_rect(Rect2(Vector2.ZERO, WINDOW_SIZE), Color(1.0, 1.0, 1.0, 0.6), false, 2.0)
