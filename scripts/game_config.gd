...
# is purely visual for now. Order matters for drawing: earlier = painted
# first (underneath).
...
# Enemy squads start at the map's far edge and advance west (decreasing x)
# toward ENEMY_ADVANCE_STOP_X before engaging. The enemy mortar deploys at a
# fixed rear position and never moves.
const ENEMY_SPAWN_X: float = 950.0
const ENEMY_SQUAD_START_Y: Array[float] = [60.0, 170.0, 280.0, 390.0, 500.0, 610.0]
const ENEMY_MORTAR_POS: Vector2 = Vector2(950.0, 335.0)
const ENEMY_ADVANCE_STOP_X: float = 420.0
...
## excluded — HIGH_GROUND is elevation-only, ROAD is decorative. BUILDING
## beats TREES if both overlap a point.
