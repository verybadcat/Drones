...
func log_blast_collateral(defender: Unit, victim: Unit) -> void:
	add_entry("The blast aimed at %s also catches %s nearby" % [defender.display_name(), victim.display_name()])
...
func log_mortar_relocating_from_density(unit: Unit) -> void:
	add_entry("%s relocates — its position was given away by recent fire, and an enemy mortar could reach it" % unit.display_name())
