...
func test_breakdown_text_lists_what_happened() -> void:
	...
	var stalemate_text: String = BattleScore.breakdown_text({"position": "stalemate"})
	check("Stalemate (neither side held the position)" in stalemate_text, "A stalemate says so, in the user's wording, rather than claiming held or lost")
	check(not ("lost" in stalemate_text), "...and never calls it lost")
	check(not ("held the position" in stalemate_text.replace("neither side held the position", "")), "...and never calls it held")
